...
function OLED(opts) {
  ...
  // grab out options, figure out what device we're dealing with
...
OLED.prototype._initialise = function() {
  ...
  var initSeq = [
    ...
    // set the last value dynamically based on screen size requirement
    this.SET_MULTIPLEX, this.screenConfig.multiplex,
    // sets offset pro to 0
    this.SET_DISPLAY_OFFSET, 0x00,
    this.SET_START_LINE,
    this.CHARGE_PUMP, 0x14,
    this.MEMORY_MODE, 0x00,
    // screen orientation
    this.SEG_REMAP,
    // screen orientation change to INC to flip
    this.COM_SCAN_DEC,
    // com pins val sets dynamically to match each screen size requirement
    this.SET_COM_PINS, this.screenConfig.compins,
    this.SET_CONTRAST, 0x8F,
    this.SET_PRECHARGE, 0xF1,
    this.SET_VCOM_DETECT, 0x40,
...
OLED.prototype._setUpSPI = function() {
  ...
  // set up spi pins
  this.dcPin = new Pin({
    pin: this.SPIconfig.dcPin,
    type: "digital",
    board: this.board
  });

  this.ssPin = new Pin({
    pin: this.SPIconfig.ssPin,
    type: "digital",
    board: this.board
  });

  this.clkPin = new Pin({
    pin: this.SPIconfig.clkPin,
    type: "digital",
    board: this.board
  });

  this.mosiPin = new Pin({
    pin: this.SPIconfig.mosiPin,
    type: "digital",
    board: this.board
  });
...
OLED.prototype.update = function() {
  ...
  this._waitUntilReady(function() {
    ...
    var displaySeq = [
      ...
      this.PAGE_ADDR, 0, (this.HEIGHT / 8) - 1
    ];
...
OLED.prototype.invertDisplay = function(bool) {
  if (bool) {
    // inverted
    this._transfer("cmd", this.INVERT_DISPLAY);
  } else {
    // non inverted
    this._transfer("cmd", this.NORMAL_DISPLAY);
  }
};
...
OLED.prototype.drawPixel = function(pixels, sync) {
  ...
  pixels.forEach(function(el) {
    ...
    byte = (page === 0) ? x : x + (this.WIDTH * page);
  ...
  if (blen > (this.buffer.length / 7)) {
    ...
    this._waitUntilReady(function() {
      ...
      for (var i = 0; i < blen; i += 1) {
        ...
        var displaySeq = [
          // column start and end address 
          this.COLUMN_ADDR, col, col,
          // page start and end address
          this.PAGE_ADDR, page, page
        ];
...
OLED.prototype.stopScroll = function() {
  this._transfer("cmd", this.DEACTIVATE_SCROLL);
};
